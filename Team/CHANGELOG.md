...
- `(this repo's hash follows in a \`changelog:\` follow-up)` -- `Team/START_HERE.md`: owner caught a leftover mistake from the 2026-09-11 identity rebuild -- this file's own "what is Control-Room" section still said it belonged to `07`/`mohammadlali0707-stack` (the deleted duplicate), missed while README.md/AGENTS.md/MAP.md/WHERE_WE_ARE.md were fixed the same day. Corrected with an explicit note pointing at the mistake, not a silent edit, and added a line on the topic router since this is the file's own "what is Control-Room" answer.
